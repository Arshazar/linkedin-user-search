...
Applies to SPEC.md writes, spec-referencing prose, backprop entries. Does NOT
apply to code, error strings, commit messages, PR descriptions.
...
> The system should ensure that every incoming request is properly authenticated
> before being forwarded to its corresponding handler function.
...
> We discovered that the token expiration check in the middleware was using a
> strict less-than comparison operator, which meant tokens were being rejected
> at the exact moment of their expiry.
...
> The POST endpoint at /x accepts a JSON body and returns a 200 response with an
> object containing the created id.
