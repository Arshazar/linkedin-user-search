...
1. **Ingest & ETL**: Parse and sanitize `300 user linkedin.csv` located at the
   root of the project (`./db/300 user linkedin.csv` relative to `/server`).
...
- **Location**: `./db/300 user linkedin.csv` (relative to `/server`)
...
CSV_FILE_PATH=./db/300 user linkedin.csv
```
